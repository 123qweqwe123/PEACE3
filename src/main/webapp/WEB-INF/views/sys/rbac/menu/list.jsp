	...
	var orderRn = "";
		...
		var option = {
			...
			colModel : [ {name : 'id',index : 'id',hidden : true},
			             {name : 'text', index : 'text', align:'left',sortable : false},
			             {name : 'menuOrder', index : 'menuOrder',hidden:true },
			             {name : 'menurn', index : 'menurn', align : 'center',sortable : false}],
...
//	}
	...
	$("#menuForm").validate({
		...
		submitHandler: function(form) {
			...
			if('更新'==btn.text()){
				...
				$.post("${ctx}/sys/rbac/menu/checkNameExists", 
		        		{ "parent":$("#parent").val(),"text":$("#menutext2").val()==''?$("#menutext").val():$("#menutext2").val()},
		        		function(data){
		        			if(data){
		        				var message;
		        				if($("#type").val()=='1'){
	...
	function add(treeId,treeNode,pNode){
		...
		$.post("${ctx}/sys/rbac/menu/getOrderCount",{parentId:pNode.id},function(result){
			if(result.success){
