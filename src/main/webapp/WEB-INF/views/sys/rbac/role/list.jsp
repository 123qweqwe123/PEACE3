...
<script type="text/javascript">
	...
	$(function(){
		...
		//	
		//	 $.ajax({
		//			url:"${ctx}/sys/rbac/role/saveRoleTail",
		//			type: "post",
		////			dataType: 'json',	
	...
	function toAdd(){
		...
		openDialog("${ctx}/sys/rbac/role/openmodalroleinput?time="+timebak);
	}	
	...
	function toModify(){
		...
		openDialog("${ctx}/sys/rbac/role/openmodalroleinput?id="+oneData.id+"&timestamp="+(new Date()).valueOf());
	}
	...
	function toDelete(){
		...
		$.post("${ctx}/sys/rbac/role/getUserRoleCount",{ids :str},function(data){
			if(data>0){
	...
	function addDate(data){
		//新增时注释id值
		//data['id'] = null;
		$.post("${ctx}/sys/rbac/role/save",data,function(result){
			$("#grid").trigger("reloadGrid");
			$('#dialog-confirm').modal('hide');
			var message = "新增失败！";
			if(result){
	...
	function updateDate(data){
		$.post("${ctx}/sys/rbac/role/update",data,function(result){
			$("#grid").trigger("reloadGrid");
			$('#dialog-confirm').modal('hide');
			var message = "更新失败！";
			if(result){
	...
	function queryTail(){
		...
		var timebak = new Date().getTime();
		//使用此方法防止js缓存不加载
		$("#dialog-tailInfo p" ).load("${ctx}/sys/rbac/role/openmodalTailDetail?id="+id+"&time="+timebak);
