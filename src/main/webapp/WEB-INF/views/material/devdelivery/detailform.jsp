...
<c:set var="ctx" value="${pageContext.request.contextPath}" />
    ...
    <form id="export_form1" class="form-horizontal">
    	<input type="hidden" name="exportLccCode" value=${exportLccCode } />
    	<input type="hidden" name="exportStockCode" value=${exportStockCode } />
		...
		<div class="form-group">
			...
			<div class="col-lg-7 col-md-8">
				<input readonly="readonly" type="text" value="${ms.storeUnit}" id="storeUnit" name="storeUnit"  class="form-control input-sm" placeholder="库存单位"/>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			<label class="col-lg-4 col-md-3  control-label" for="exportAmount">出库数量:</label>
			<div class="col-lg-7 col-md-8">
				<input type="text" id="exportAmount" name="exportAmount" readonly="readonly"  value="1" class="form-control input-sm" placeholder="出库数量"/>
			</div>
		</div> 
	...
	<script type="text/javascript">
		$(function(){
			...
			//物资名称
			$.getJSON("${ctx}/material/devdelivery/comboxData?stockCode="+stockCode,function(data) { 
				$('#materlName').autocomplete(data,{
			       ...
			       // 下拉列表显示的字段 ，如果多个用表格格式化，如果单个 直接  return item.COUNTRY_NAME; 即可 
			        formatItem: function(item,i, max) {
			        	if(item){
			        		 return '<table><tr><td width="100px;">' + item.materlName+'/</td><td width="50px;">'+item.materlBatch+
			        		 '/</td><td width="70px;">'+item.wholesalePrice+
