...
<body>
	...
	<ul class="breadcrumb">
		...
		<li class="active">物资基本信息</li>
	</ul>
	<form class="well bs-adp form-inline" id="searchForm">
	       	物资名称:
	        <input type="text" id="materlName" name="materlName" class="form-control input-sm"/>
	    &nbsp;&nbsp;&nbsp;&nbsp; 
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="dialog-confirm-title">物资基本信息</h4>
				</div>
	...
	<script type="text/javascript">
		$(function() {
			...
			var option = {
				url : '${ctx}/material/info/list',
				datatype : 'json',
				mtype : 'POST',
				colNames : [ '','', '物资名称', '库存单位', '规格', '生产厂家', '拼音简码', '创建日期','创建人','是否有效', '物资类别名称' ],
				colModel : [
						{
							name : 'materlInfoCode',
			...
			.navButtonAdd('#pager',{caption:"置为无效",buttonicon:"ui-icon-trash",onClickButton: function(){toDelete()},position:"last"});
			//自适应
		...
		function toAdd(){
			...
			openDialog("${ctx}/material/info/openmodaladdmaterinfoinput?time="+timebak);
		}
		function toModify(){
			var materlInfoCode = $("#grid").jqGrid('getGridParam','selrow');
			if($.isEmptyObject(materlInfoCode)) {
				openError('请选择要编辑的记录！',2000);
				return;
			}
			var timebak = new Date().getTime();
			openDialog("${ctx}/material/info/openmodaladdmaterinfoinput?materlInfoCode="+materlInfoCode+"&time="+timebak);
		}
		$('#do_save').click(function(){
			...
			if(data.materlInfoCode ==""){addDate(data);}
			else{updateDate(data);}
		});
		
		function addDate(data){
			$.post("${ctx}/material/info/addMaterlInfo",data,function(result){
		...
		function toDelete(){
			var materlInfoCode = $("#grid").jqGrid('getGridParam','selrow');
			var rowData = $("#grid").jqGrid('getRowData',materlInfoCode);
			if(rowData.isRemoved.indexOf('无效')>0){
				openError('该物资已经是无效状态！',2000);
				return;
			}
			if($.isEmptyObject(materlInfoCode)) {
		...
		$('#do_delete').click(function(){
			...
			//开始执行删除动作
			$.post("${ctx}/material/info/delete", 
					{materlInfoCode :materlInfoCode },
        		function(data){
					$("#grid").trigger("reloadGrid");
					$('#dialog-delete').modal('hide');
					var message = "删除失败！";
