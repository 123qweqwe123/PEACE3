...
<body>
	...
	<div id="select">
		<div class="select-main">
			<form action="" id="searchForm" method="post" class="well-work bs-adp form-inline">
				...
				<div  class="row" style="margin-top:5px;"> 
					<div  class="leftLable col-md-1">退订时间:</div> 
					<div  class="right col-md-2"> 
						<input name="replyTime" id="replyTime"  class="form-control input-sm" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})"/>
					</div>
	...
	$(function(){
		...
		var option = {
	        ...
	        datatype : 'json',  
	        mtype : 'POST',
	        colNames : [ 'LCCID','PID','患者姓名','身份证','手机','所属组别','短信回复内容','回复时间','短信交互'],
	        colModel : [ 
		...
		.navButtonAdd('#pager',{caption:"审核通过",buttonicon:"ui-icon-check",onClickButton: function(){toAudit()},position:"last"})
	...
	function toAudit(){
	    ...
	    for(var i in _ids){
	    	...
	    	pIds+=rowData.patientId;
	    }
	    jConfirm("确认审核通过这"+_ids.length+"条数据吗?",null,function(r){
	    	if(r){
	    		$.post("${ctx}/msg/td/audits?pIds="+pIds,null,function(result){
					if(result && result !='undefinded' &&result.success==true){
						$("#grid").trigger('reloadGrid');
	        	    	showResult(true,'审核成功');
					}else{
						showResult(false,'审核失败');
					}
				},'json');
	    	}
	    });
