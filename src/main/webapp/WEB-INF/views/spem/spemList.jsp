...
<ul class="breadcrumb">
  <li class="active">样本管理</li><li class="active" >中心样本通知调运管理</li>     
</ul>    
  
<form class="well bs-bdcor form-inline" id="searchForm">
       <div  class="row"> 
        <div  class="col-lg-2 col-xl-2 col-md-offset-1 col-md-2"> 
                  运送单号: 
        </div> 
        <div  class="col-lg-2 col-xl-2  col-md-2"> 
              <input type="text"     class="form-control input-sm" name="waybill_no" id="waybill_no" value='' placeholder="输入关键字 模糊查询"/>
        </div> 
        <div  class="col-lg-2 col-xl-2   col-md-2">
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;冻存盒号:
       </div> 
...
var option = {
        ...
        colModel : [ 
                     {name : 'lcc_name', index : 'l.lcc_name',  align:'center',sortable: false  },
                     {name : 'waybill_no',index : 'sw.WAYBILL_NO', align:'center'}, 
                     {name : 'projectId', index : 'projectId',  align:'center',hidden : true}, 
                     {name : 'boxCodes', index : 'boxCodes',  align:'center',sortable: false,hidden : true  },
...
function  Gosub(){
        ...
        for(var id in ids)
        {
        	...
        	var waybill_no = $("#grid").getCell( ids[id] , 'waybill_no');
        	...
        	if(value==1){
            	...
            	//var btn = "<button type='button' onclick='completeCurrentStatus("+value+")' class='btn btn-primary btn-xs' >发起请求</button>";
                var btn = "<button type='button' onclick='completeCurrentStatus("+ids[id]+")' class='btn btn-primary btn-xs disabled' >等待处理</button>";
            	...
            	//var btn = "<button type='button' onclick='completeCurrentStatus("+value+")' class='btn btn-primary btn-xs' >发起请求</button>";
                var btn = "<button type='button' onclick='completeCurrentStatus("+ids[id]+")' class='btn btn-primary btn-xs disabled' >已完成</button> ";
                $("#grid").setCell(ids[id],'op',btn, {height:35} ); 
            }    
        }  
 }
...
$('#do_delete').click(function(){
    ...
    var waybill_no =  $("#grid").getCell( _ids , 'waybill_no');
    
    
    $("#do_delete").attr("disabled",true);
    
    //开始执行删除动作
    $.post("${ctx}/spem/waybill/delete",    
            {id :waybill_no }, 
        function(data){
