...
$(function() {
	...
	var option = {
			url : '${ctx}/progress/reportCompletenesslist',
			postData :o ,
			datatype : 'json',
			mtype : 'POST',
			colNames : [ '项目点编码', '项目点名称', '知情同意书' ,'心电图', '颈动脉超声', '心脏超声', '血样采集', '尿样采集',
					'高危调查问卷','高危核查表','全部高危项目','随访血压','随访体重','心电图','随访调查问卷' ],
			colModel : [
					{
						name : 'LCC_CODE'
					},
					...
					},
					{
						...
						formatter : function(cellvalue, option, rowObjects) {
							if ( cellvalue )
								return cellvalue+'/'+rowObjects['ECG_PER']+'%';
							return '0/0%';
						}
					},
					{
						...
						formatter : function(cellvalue, option, rowObjects) {
							if ( cellvalue )
								return cellvalue+'/'+rowObjects['CCH_PER']+'%';
							return '0/0%';
						}
					},
					{
						...
						formatter : function(cellvalue, option, rowObjects) {
							if ( cellvalue )
								return cellvalue+'/'+rowObjects['ECH_PER']+'%';
							return '0/0%';
						}
					},
					{
						...
						formatter : function(cellvalue, option, rowObjects) {
							if ( cellvalue )
								return cellvalue+'/'+rowObjects['BLOOD_PER']+'%';
							return '0/0%';
						}
					},
					{
						...
						formatter : function(cellvalue, option, rowObjects) {
							if ( cellvalue )
								return cellvalue+'/'+rowObjects['URINE_PER']+'%';
							return '0/0%';
						}
					},
					{
					...
					},
					{
						...
						formatter : function(cellvalue, option, rowObjects) {
							if ( cellvalue )
								return cellvalue+'/'+rowObjects['BP_PER']+'%';
							return '0/0%';
						}
					},
					{
						...
						formatter : function(cellvalue, option, rowObjects) {
							if ( cellvalue )
								return cellvalue+'/'+rowObjects['WEIGHT_PER']+'%';
							return '0/0%';
						}
					},
					{
						...
						formatter : function(cellvalue, option, rowObjects) {
							if ( cellvalue )
								return cellvalue+'/'+rowObjects['INTERVIEWECG_PER']+'%';
							return '0/0%';
						}
					},
					{
			...
			loadComplete : function() {
				...
				var _data = {'LCC_CODE':' ','LCC_NAME':'合计/统计','UQS0':0,'UQS34':0,'RISK':0,'UQS3':0,'UQS6':0,'INTERVIEW':0};
				for(var i = 0; i < ids.length;i++){
					var _id = ids[i];
					var r = jQuery("#grid").jqGrid('getRowData',_id);
					_data['UQS0'] += parseInt(r['UQS0']);
					_data['UQS34'] += parseInt(r['UQS34']);
					_data['UQS3'] += parseInt(r['UQS3']);
					_data['UQS6'] += parseInt(r['UQS6']);
					_data['INTERVIEW'] += parseInt(r['INTERVIEW']);
				}
				
				if ( _data['UQS2'])
					_data['UQS2_PER'] =  Math.round( _data['UQS2'] / 4000) ;
				else
					_data['UQS2_PER'] = 0 ;
				
				if ( _data['UQS2'] && _data['UQS2'] > 0 &&  _data['RISK'])
					_data['RISK_PER'] =  Math.round( _data['RISK'] * 100 / _data['UQS2'] ) ;
				else
					_data['RISK_PER'] = 0;
				if (_data['UQS34'])
					_data['UQS34_PER'] =  Math.round( _data['UQS34'] / 800) ;
				else
					_data['UQS34_PER'] = 0;
				if ( _data['UQS5'] && _data['UQS34'] && _data['UQS34'] > 0 )
					_data['UQS5_PER'] =  Math.round( _data['UQS5'] * 100 / _data['UQS34']) ;
				else
					_data['UQS5_PER'] = 0;
				if (_data['UQS6'] && _data['UQS5'] && _data['UQS5'] > 0 )
					_data['UQS6_PER'] =  Math.round( _data['UQS6'] * 100 / _data['UQS5'] ) ;
				else
					_data['UQS6_PER'] = 0;
				
				if (_data['INTERVIEW'] && _data['UQS6'] && _data['UQS6'] > 0 )
					_data['INTERVIEW_PER'] =  Math.round( _data['INTERVIEW'] * 100 / _data['UQS6'] ) ;
				else
					_data['INTERVIEW_PER'] = 0;
		...
		$('#exportQuery').click(function(){
			var url = './exportexcel?template=completeness';
			if ( $('#bg_date').val() != '')
