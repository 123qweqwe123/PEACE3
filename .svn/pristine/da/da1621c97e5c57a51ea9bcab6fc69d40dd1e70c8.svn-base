...
<div id="wrap">
	...
	<div id="main-content">
		<div id="left-main" style="width:99%">
		...
		<c:forEach items="${paper.QGroup }" var="qg">
			 ...
			 <c:forEach items="${qg.qs }" var="q">
				<c:choose>
					...
					<c:otherwise>
						<div thisTitle="${q.helpText}" displayname="${q.displayname}" updateReadOnly="${q.updateReadOnly}"
...
<br style="clear:both" />
<div class="btn-wrap">
       <button type="button" id="bnt"  onclick="sub()" class="btn-style-default">上报</button>
</div>
		...
		<c:if test="${not empty errorMsg }"><p style="color:red;font-size:40px;">${errorMsg}</p></c:if>
	</div>
	<!-- /container -->
	<input type="hidden" id="loginName"/>
	<input type="hidden" id="loginPwd"/>
<form action="${ctx}/qn/eventMgt/saveQn" method="post" id="subForm">
</form>
...
function init(){
	if("${isModify}" != ""){
		$("div[updateReadOnly='1']").find("input").attr("disabled","disabled");
	}
...
function sub(){
	...
	$("#subForm").append("\n<input type=\"hidden\" name=\"uqsTypeName\" value=\"${uqsTypeName}\"/>");
...
function triggerShowOrHide(optionInputId){
	...
	//allEnd
	$("div[allEnd*='_']").each(function(){
		allEnd(optionInputId,$(this)[0].checked);
	});
...
function showOrHide(obj){
	...
	if(controlShow != undefined && controlShow != ""){
		...
		if(eval(convertControlShow) && $(obj).parent().is(":visible") && $(this).attr("isEnd")!=1){
			$(obj).show();
		}else{
			$(obj).hide();
		}
	...
	if(controlHide != undefined && controlHide != ""){
		...
		if(!eval(convertControlHide) && $(obj).parent().is(":visible")  && $(this).attr("isEnd")!=1){
			$(obj).show();
		}else{
			$(obj).hide();
		}
