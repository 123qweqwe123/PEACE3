...
<body>
	...
	<script type="text/javascript">
		...
		//弹出新增对话框			
		function toAdd(){
			var timebak = new Date().getTime();//1:NCC  2:RCC    3:LCC
			openDialog("${ctx}/pro/lccuser/openmodallccuserinput?lccRoleType=3&time="+timebak);
		}	
		function toModify(){
			...
			openDialog("${ctx}/pro/lccuser/openmodallccuserinput?lccRoleType=3&userCode="+userCode+"&time="+timebak);
		}
		...
		function outExcelLccUser(){
			...
			window.location.href ='${ctx}/pro/lccuser/outExcelLccUser?lccRoleType=3';
		}
