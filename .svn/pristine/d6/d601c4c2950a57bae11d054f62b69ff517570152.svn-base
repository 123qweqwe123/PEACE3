...
<body>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="dialog-confirm-title">物资名称</h4>
				</div>
	...
	<script type="text/javascript">
		$(function() {
			...
			var option = {
				url : '${ctx}/material/name/list',
				datatype : 'json',
				mtype : 'POST',
				colNames : [ '', '物资名称', '拼音简码', '创建日期', '创建人' ],
				colModel : [
						{
							name : 'materlCode',
							index : 'MATERL_CODE',
		...
		function toAdd(){
			...
			openDialog("${ctx}/material/name/openmodaladdmaternameinput?time="+timebak);
		}
		function toModify(){
			var materlCode = $("#grid").jqGrid('getGridParam','selrow');
			if($.isEmptyObject(materlCode)) {
				openError('请选择要编辑的记录！',2000);
				return;
			}
			var timebak = new Date().getTime();
			openDialog("${ctx}/material/name/openmodaladdmaternameinput?materlCode="+materlCode+"&time="+timebak);
		}
		$('#do_save').click(function(){
			...
			if(data.materlCode ==""){addDate(data);}
		...
		$('#do_delete').click(function(){
			...
			//开始执行删除动作
			$.post("${ctx}/material/name/delete", 
					{materlCode :materlCode },
        		function(data){
					$("#grid").trigger("reloadGrid");
					$('#dialog-delete').modal('hide');
					var message = "删除失败！";
