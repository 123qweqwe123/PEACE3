    ...
    <div id="main-content">
        <div id="left-main">
                                                            ...
                                                            <div class="col-sm-6" controlshow="${option.controlShow}" controlhide="${option.controlHide}">
                                                                ...
                                                                <label for="o_${qg.id}_${q.id}_${option.id}">
                                                                    <input
                                                                            ...
                                                                            <c:if test="${option.readOnly == '1'}">disabled</c:if>
            ...
            <div class="btn-wrap noPrint">
                <span><button type="button" id="bnt"  onclick="sub()" class="btn-style-default">提交问卷</button></span>
            </div>
            ...
            <div class="col-md-2">
                <div class="bs-docs-sidebar hidden-print hidden-xs hidden-sm affix" role="complementary" style="OVERFLOW-Y: auto; OVERFLOW-X:hidden;max-height:400px;margin-top:-50px;">
                    <ul class="nav bs-docs-sidenav">
                        <c:forEach items="${paper.QGroup}" var="qs">
...
<!-- /container -->
<form action="${ctx}/changeans/saveChanges" method="post" id="subForm">
</form>
...
<script type="text/javascript">
    ...
    function blurRule(obj,textType,format,scope,constrainScope){
        ...
        if( "${uqsVersion}".substr(4,3)=="001" ){
            var bloodNo = $("input[name='4_3_1']:visible").val();

            if( src_bloodNo != bloodNo  ){
                if(bloodNo != undefined && $.trim(bloodNo) != ''){
                    var data = {};
                    data['bloodNo']=bloodNo;
                    data['pid'] = "${patientId}";
                    data['uqsver'] = "${uqsVersion}";
                    if(bloodNo != undefined && $.trim(bloodNo) != ''){
                        var back=false;
                        $.post("${ctx}/changeans/checkBloodNo",data,function(result){
                            if(!result.success){
                                jAlert(result.msg);
                                back=true;
                            }
    ...
    window.onload = function (e) {
        ...
        src_bloodNo = $("input[name='4_3_1']:visible").val();
...
<script type="text/javascript">
    ...
    function sub(){
        ...
        $("#subForm").append("\n<input type=\"hidden\" name=\"uqsfile\" value=\"${uqsfile}\"/>");
...
<script type="text/javascript" >
    ...
    function surePwd(){
        var surePwdHtml = "帐号:<input type='text' onblur='cLoginName(this)'/><br/><br/>密码:<input type='password'" +
                " onblur='cPwd(this)'/>";
        $("#loginName").val("");
        $("#loginPwd").val("");
