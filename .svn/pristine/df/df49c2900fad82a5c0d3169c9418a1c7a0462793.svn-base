...
<ul class="breadcrumb">
  <li class="active">系统管理</li><li class="active" >权限管理</li>
</ul>
<div class="row">
		...
		<div id="form_div" hidden>
			<form id="permissionForm">
				<fieldset>
					...
					<div class="form-group">
						<label for="isResource">资源类型:</label>
						<select id="isResource" name="isResource" name="isResource" class="form-control input-sm">
...
<script type="text/javascript">
	...
	function addHoverDom(treeId, treeNode) {
		...
		<shiro:hasPermission name="permissionmgt:add">
		...
		if(treeNode.isResource=='4'){
			$("#addBtn_"+treeNode.id).hide();
		}
		</shiro:hasPermission>
		<shiro:hasPermission name="permissionmgt:update">
		...
		<shiro:hasPermission name="permissionmgt:delete">
		if(treeNode.grade!=0){
...
/* 	function showRemoveBtn(treeId, treeNode) {
    ...
    function onRename(e, treeId, treeNode,isCancel) {
        ...
        $.post("${ctx}/sys/rbac/permission/rename", 
    ...
    function remove(){
    	...
    	var id = $("#del_id").val();
    	$.post("${ctx}/sys/rbac/permission/delete", 
        		{ "id": id},
        		function(data){
        			var message = "删除失败！";
        			if(data){
    ...
    function onUpdate(){
    	...
    	$.post("${ctx}/sys/rbac/permission/update", 
        		{ "id": $("#id").val(),
    				'name':$("#name").val(),
    				'code':$("#code").val(),
    				'permissionType':$("#permissionType").val(),
    				'url':$("#url").val(),
    				'isResource':parseInt($("#isResource").val()),
    				'remark':$("#remark").val()},
        		function(data){
        			var message = "更新失败！";
        			if(data){
    ...
    function editOnClick(event, treeId, treeNode) {
    	...
    	if(treeNode.isResource != '0'){
    		//$('#isResource')[0].selectedIndex = parseInt(treeNode.isResource); 
    		$("#isResource").val(treeNode.isResource);
    	}
	...
	$("#permissionForm").validate({
		...
		submitHandler: function(form) {
			...
			if('更新'==btn.text()){
				...
				$.post("${ctx}/sys/rbac/permission/checkNameExists", 
	        		{ "parentId":$("#parentId").val(),"name":$("#name").val()},
	        		function(data){
	        			if(data){
	        				message = "名称已经存在，请重新输入！";
	        				showResult(false,message);
	        				$("#permissionForm :button").attr("disabled",false);
	        			}else{
	        				var options = {
	        			    	 	success:showResponse,
	        			    	 	url:'${ctx}/sys/rbac/permission/add',       
	        			            type:'post'	 
	        			    	 };
	...
	function add(treeId,treeNode,pNode){
		...
		$("#permissionForm").attr('action','${ctx}/sys/rbac/permission/add').attr('method','POST');
	}
