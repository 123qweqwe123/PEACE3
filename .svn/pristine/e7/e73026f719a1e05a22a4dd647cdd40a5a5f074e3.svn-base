...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#do_save1").click(function () {
            $.post("${ctx}/qn/eventMgt/checkEvents/addAuditUser", function (data) {
                if (data) {
                    $('#dialog-event-add').modal('hide');
                    $("#sButton").show();
                } else {
    ...
    // 返回到列表页面
    function returnMain() {
        window.location.href = '${ctx}/qn/eventMgt/checkEvents'
    }
        ...
        if (password != null) {
            $.post("${ctx}/sys/rbac/user/checkConfirmOldPassword", {password: password}, function (data) {
                if (data.result) {
                    ...
                    $.post("${ctx}/qn/eventMgt/checkEvents/saveEvent", data, function (d) {
                        if (d == "success") {
                            $("#submitMsg").text("保存成功!")
                            setTimeout(function () {
    ...
    function initEr() {
        ...
        if (code != "") {
            $("#er").load("${ctx}/qn/eventMgt/openmodalcurrdescer?code=" + code + "&eventCode=" + eventCode)
        }
    }
    ...
    function addEvent(eventCode) {
        $.post("${ctx}/qn/eventMgt/checkEvents/addEvent?eventCode=" + eventCode, function (data) {
            if (data.success) {
                window.open("${ctx}/qn/eventMgt/checkEvents/check?eventCode=" + data.eventCode)
            } else {
